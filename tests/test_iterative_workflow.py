...
class IterativeWorkflowTests(unittest.TestCase):
    def _make_result(self) -> AnalysisResult:
        ...
        finding = ServiceFinding(
            ...
            ai_confidence=0.45,
            command_suggestions=[
                CommandSuggestion(command="whatweb http://TARGET", source="playbook", confidence=0.8, risk="low"),
                CommandSuggestion(command="whatweb http://TARGET", source="ai", confidence=0.45, risk="low"),
                CommandSuggestion(command="curl -I http://TARGET", source="ai", confidence=0.45, risk="low"),
            ],
            risk_score=7.5,
